...
4) Significance of the fixed effects (slope != 0?) is tested using t-tests with Satterthwaite’s approximation method.
...
# Create additional columns for subsequent memory measures of interest
behavData$Sound[behavData$EmotionCorrect == 1 & behavData$EmotionConfidence == 1] <- 1 # only sucessful if correct and high confidence
behavData$Sound[behavData$EmotionCorrect == 1 & behavData$EmotionConfidence == 0] <- 0
behavData$Sound[behavData$EmotionCorrect == 0] <- 0

# define 'success' thresholds for color and scene trials -> 75% probability within von Mises
cCutOff = 42
sCutOff = 24

# feature accuracy
behavData$Color[behavData$ColAbsError <=cCutOff] <- 1  #correct if error (how far from target) less than or equal to threshold
behavData$Color[behavData$ColAbsError > cCutOff] <- 0
behavData$Scene[behavData$SceAbsError <=sCutOff] <- 1 #correct if error (how far from target) less than or equal to threshold
behavData$Scene[behavData$SceAbsError > sCutOff] <- 0
...
#### separate trial-unique conditions:
## 1 feature
...
behavData$OneFeature.Only <- behavData$Color.Only + behavData$Sound.Only + behavData$Scene.Only
...
# ****now format behavioral data to match single trial beta ENCODING order**** -- by encoding onset:
behavData <- behavData[order(behavData$StudyTrial),]
behavData <- data.frame(behavData[order(behavData$SubID),])
...
kable(feature.accuracy %>% gather(Feature,Accuracy,Color:Scene) %>%
                          group_by(Feature) %>%
                          summarise(Memory = mean(Accuracy)),format="pandoc",
...
allData.clean <- allData.clean[,-c(4)] # remove vox numbers - not needed here
...
# clean up behavioral data to match myBetas
behavData.clean <- behavData[idxBetas,]

testData <- cbind(myBetas.clean,behavData.clean)
...
offset.data <- testData #store as offset betas
offset.behav <- behavData.clean #store as matching offset behavioral data
...
mixed.data <- data.frame(array(0,c(length(rois)*2,7)))
colnames(mixed.data) <- c('ROI','Phase','Feature','Beta','SE','T','Pvalue')
row = 0
...
for (phase in 1:2) {
  ...
  if (phase == 1) {
    model.data <- gather(onset.data, ROI, Beta, 1:length(rois))
    time = 'Onset'
  } else if (phase == 2) {
    model.data <- gather(offset.data, ROI, Beta, 1:length(rois))
    time = 'Offset'
  }
  ...
  for (r in 1:length(rois)) {
    ...
    mixed.data[(row+1):(row+nIV-1),c("Beta","SE","T","Pvalue")] <- stats[2:(nIV),c("Estimate","Std. Error","t value","Pr(>|t|)")]
...
### set up data frame for mixed effect model results:
mixed.data <- data.frame(array(0,c(length(rois)*6,7)))  # 3 features x 2 times
colnames(mixed.data) <- c('ROI','Phase','Feature','Beta','SE','T','Pvalue')
row = 0
...
####################################
mixed.data <- data.frame(array(0,c(length(newRois)*2,6)))
colnames(mixed.data) <- c('ROI','Feature','Beta','SE','T','Pvalue')
row = 0
...
colnames(mixed.data) <- c('Phase','Feature','Beta','SE','T','Pvalue')
row=0
